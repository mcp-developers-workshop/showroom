...
Quarkus provides a dev mode which greatly aids during development by providing a number of features in keeping with the mantra of providing developer joy. Some of the features include:
...
. Open a new terminal window on the VS Code.
+
...
. Select `/hello` endpoint on the Quarkus welcome. You should see the following page:
+
image::crw-open-page-hello.png[page, 800]

. When you fail to access the *hello* endpoint URL, make sure if the newly opened tab has the *http* protocol.
