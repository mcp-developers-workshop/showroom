...
A bug has been reported where the last letter is not working. To reproduce the bug, try to retrieve the last letter of the string `Foo` by running this in a Terminal (your app should still be running - if it is not, use _Start Live Coding_ again):
...
To debug the app, let's step through the function that has the bug. In the left gutter of the code, to the left of the line numbers, select once next to `int len = name.length();` to set a breakpoint and cause a red circle to appear:
...
* **Step Over** again, which executes the line to grab the last letter using `len` as an offset to the `substring` method. See the bug? Look at the value of `lastLetter` in the variables list on the right - it's empty!
