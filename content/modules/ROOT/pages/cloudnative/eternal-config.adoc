...
Then, you will see the *Property greeting.message updated*. Make sure that the _application.properties_ file is updated automatically:
...
We also need to update the functional test to reflect the changes made to the endpoint. Edit the `src/test/java/org/acme/people/GreetingResourceTest.java` file and change the content of the `testHelloEndpoint` method to:
